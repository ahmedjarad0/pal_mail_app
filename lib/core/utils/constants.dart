import 'dart:ui';


// ****************images************************
const String palestineAsset = 'assets/img/palestine_bird.png';
const String facebookImage = 'assets/img/facebook.svg';
const String googleImage = 'assets/img/google.svg';
const String twitterImage = 'assets/img/twitter.svg';
const String appBarImage = 'assets/img/person.svg';
const String manImage = 'assets/img/man.svg';
// ****************colors************************

const kPrimaryColor = Color(0xff003AFC);
const kLightPrimaryColor = Color(0xff587FFF);
const kScaffoldColor = Color(0xffF7F6FF);
const kAppBarColor = Color(0xff6589FF);
const kHintColor = Color(0xffB2B2B2);
// ****************apiUrls************************
const String baseUrl = 'https://palmail.gsgtt.tech/api/';
const String loginUrl = '${baseUrl}login';
const String registerUrl = '${baseUrl}register';
const String logoutUrl = '${baseUrl}logout';
const String userUrl = '${baseUrl}user';
const String tagsUrl = '${baseUrl}tags';
const String mailsUrl = '${baseUrl}mails';
const String categoriesUrl = '${baseUrl}categories';
const String statusesUrl = '${baseUrl}statuses?mail=true';

